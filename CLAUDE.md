...
`engine/source-watcher.js` (process Railway `sources`, voir `engine/Procfile`) lit `engine/sources.json` et transforme chaque nouvel article RSS en événement `detected`/`unverified` via `sa-events/v1`. Aucun flash ni article n'est jamais créé automatiquement à partir de la veille. Sources actives au 2026-09-17 — 8 sources actives, toutes avec `filter_languages: true` (arabe / français uniquement) :
...
**Sport / Football :** MarocFoot (syndication ✅, domaine migré vers marocfoot.net), Foot Mercato (syndication ✅, URL /flux-rss, joueurs marocains en Europe), AlMaghrebSport (syndication ✅, arabe, Botola/Lions de l'Atlas/Wydad/Raja), Hespress Sport (syndication ✅, arabe, rubrique sport Hespress). Sources désactivées (6) : L'Equipe (403), Le360 Sport (404), Kooora (DNS ENOTFOUND), Goal.com Arabe (404), FilGoal (timeout/blocage), Yalla Kora (HTML au lieu de RSS).
