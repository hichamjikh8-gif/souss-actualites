...
- `souss-actualites` (WordPress) : `WORDPRESS_DB_HOST`, `WORDPRESS_DB_NAME`, `WORDPRESS_DB_USER`, `WORDPRESS_DB_PASSWORD`, `WORDPRESS_CONFIG_EXTRA` (contient `SOUSS_BOT_SECRET`, et optionnellement `FACEBOOK_PAGE_ID` / `FACEBOOK_PAGE_ACCESS_TOKEN` / `X_BEARER_TOKEN` si la publication reelle Facebook/X est activee, `PEXELS_API_KEY` / `UNSPLASH_ACCESS_KEY` pour la recherche automatique d'image a la une, et `ANTHROPIC_API_KEY` pour le resume automatique des articles publies - toutes ces cles peuvent aussi etre posees comme simple variable d'environnement Railway plutot que comme `define()`, le code accepte les deux, voir `sa_event_secret()`).
